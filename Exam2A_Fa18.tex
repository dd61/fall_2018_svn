...
\begin{document}
	...
	\extraheadheight{-.4in}
	\runningheadrule
	\firstpageheader{\bfseries {MATH1-UC 1171}}{ \bfseries {Exam 2A }}{\bfseries {12/18/2018}} 
	...
	\addplot[gray!30] fill between[of=A and B,soft clip={domain=2:2.3}];
	\node [rotate=40] at (axis cs:  0.95,  .69) {$y=x$};
	\node [rotate=-69] at (axis cs:  2.95,  .79) {$y=9-3x$};
	\node [rotate=0] at (axis cs:  0.95,  3.19) {$y=3$};
	\end{axis}
	\end{tikzpicture}
...
\question This question has \setcountertopartsofq{\arabic{question}}\underline{\wordnum{numpofq}} (\arabic{numpofq}) parts ((a)---(\alph{numpofq})).
...
\newpage		
		...
		\begin{parts}
			...
			\part[2] Find the exact times when capsule 1 is at a height of 70 meters during the first rotation ($0\leq t \leq 24$).
			
			\fillwithdottedlines{3.5cm}
			\part[1] If we restrict the domain of $H$ to the first half of the first rotation ($0\leq t \leq 12$) then the function $H$ becomes one-to-one. The units of the number \enquote{60} in $\displaystyle H^{-1}(60)$ are: \begin{oneparchoices}
...
\question[2] Find the exact solution of the equation: $\displaystyle 2e^{16x}=19$.
				
\fillwithdottedlines{1.5cm}
				
\question[2] Find the exact solution of the equation: $\displaystyle \log(x) + \log(x - 5) = \log(5x)$.
				
\fillwithdottedlines{2cm}

\question[2] Write the exact values of the two solutions of the equation $\displaystyle \sin x = 0.9$ in $[0,2\pi]$.
...
Find the present value of \$10,000 if interest is paid at a rate of 0.8\% per year, compounded semiannually, for 2 years. Write Your answer in exact form.

\fillwithdottedlines{2cm}		

\question[2] A quantity, called Quantity A, starts at a size of 100 units (at $t=0$ minutes) and grows exponentially. After 1 minute, the size of Quantity A is 102 units. Find the number of minutes it takes for Quantity A to double.    Show all Your work step-by-step and give Your answer in exact form. Note: You do not have to provide a decimal approximation of Your answer.
...
\question[2] Another quantity, called Quantity B, starts at a size of 100 units (at $t=0$ minutes) and decreases exponentially. After 1 minute, the size of Quantity A is 98 units. Find the number of minutes it takes for Quantity B to reach a size of 50 units.     Show all Your work step-by-step and give Your answer in exact form. Note: You do not have to provide a decimal approximation of Your answer.

\fillwithdottedlines{2cm}
\bonusquestion[1]
		True or False? If the statement is always true, write True in the space provided below; otherwise, write False.
			 $\displaystyle \sin^{-1}\left ( \sin \left ( \frac{11\pi}{4}\right ) \right ) = \frac{11\pi}{4}$ \dotfill
...
\begin{parts}
		\begin{multicols}{2}
	\part[1] $\displaystyle \sin \left ( -\frac{13\pi}{4} \right ) $ \dotfill
	\part[1] $\displaystyle \sin^{-1} \left ( -\frac{13\pi}{4} \right ) $ \dotfill
	\bonuspart[1] $\displaystyle \cos(\pi)$ \dotfill
	\bonuspart[1] $\displaystyle \sin \left (\frac{\pi}{2}\right )$ \dotfill
	\part[1] $\displaystyle \sin^{-1}\left (-\frac{1}{2}\right )$ \dotfill
	\part[1] $\displaystyle \cos^{-1}\left (-\frac{1}{2}\right )$ \dotfill	
		\end{multicols}
	\end{parts}	
...
\begin{parts}
	\begin{multicols}{2}
		...
		\part[1] $\displaystyle \frac{\log x}{\log y}=\log x - \log y$ \dotfill
		
		\part[1] $\displaystyle \left(\log \left( x\right)\right)^3=3\left(\log \left( x\right)\right)$\dotfill
		
		\part[1]  $\displaystyle \log_2 \left( \frac{1}{2} \right) = -1$ \dotfill
		
		\part[1]  $\displaystyle \log(0)=1$ \dotfill
		
		\part[1] $\displaystyle \log(0.1)=-1$ \dotfill
		
		\part[1] $\displaystyle \log(1)=0$ \dotfill
...
\begin{center}
	...
	%	\tcaption{One period  of $y = f(x)$.}
		...
		\dotted \function{-2, -1, 0.1}{2*cos(3.14159265*(x+1)/3)}
		\dotted \function{5, 6, 0.1}{2*cos(3.14159265*(x+1)/3)}
		\function{-1, 5, 0.1}{2*cos(3.14159265*(x+1)/3)}
	\end{mfpic}
