...
\begin{document}
	...
	\extraheadheight{-.4in}
	\runningheadrule
	\firstpageheader{\bfseries {MATH1-UC 1171}}{ \bfseries {Exam 2B }}{\bfseries {12/18/2018}} 
...
%\end{tikzpicture} 		
 ...
 ((a)---(\alph{numpofq})).
   ...
   Each time your heart beats, your blood pressure first increases and then decreases as the heart rests between beats. The maximum and minimum blood pressures are called the systolic and diastolic pressures, respectively. Your blood pressure reading is written as systolic/diastolic. A reading of 120/80 is considered normal. A certain person's blood pressure is modeled by the function
 ...
 \begin{parts}
 	...
 	\part[2] Find the person's maximum (systolic) blood pressure.
 	\fillwithdottedlines{1.5cm}
 \end{parts}
...
\begin{parts}
		...
		\part[2] Write the equation of $H(t)$ using the cosine function, that is, write $H(t)$ as $\displaystyle a\cos(B(x-h))+k$.
		\fillwithdottedlines{2cm}
		
		\part[2] Find the exact times when capsule 1 is at a height of 75 meters during the first rotation ($0\leq t \leq 24$).
		
		\fillwithdottedlines{3.5cm}
		\part[1] If we restrict the domain of $H$ to the first half of the first rotation ($0\leq t \leq 12$) then the function $H$ becomes one-to-one. The units of the number \enquote{65} in $\displaystyle H^{-1}(65)$ are: \begin{oneparchoices}
...
\question[2] Find the exact solution of the equation: $\displaystyle \log(x) + \log(x - 4) = \log(4x)$.
...
\fillwithdottedlines{2cm}	
		...
		\begin{parts}
			\begin{multicols}{2}
				\part[1] $\displaystyle \sin \left ( -\frac{15\pi}{4} \right ) $ \dotfill
				\part[1] $\displaystyle \sin^{-1} \left ( -\frac{15\pi}{4} \right ) $ \dotfill
				\bonuspart[1] $\displaystyle \cos(2\pi)$ \dotfill
				\bonuspart[1] $\displaystyle \sin \left (\frac{3\pi}{2}\right )$ \dotfill
				\part[1] $\displaystyle \sin^{-1}\left (-\frac{\sqrt{2}}{2}\right )$ \dotfill
				\part[1] $\displaystyle \cos^{-1}\left (-\frac{\sqrt{2}}{2}\right )$ \dotfill	
			\end{multicols}
		\end{parts}	
		...
		\begin{parts}
			\begin{multicols}{2}
				...
				\part[1] $\displaystyle \log x - \log y=\frac{\log x}{\log y}$ \dotfill
				
				\part[1] $\displaystyle \left(\log \left( x\right)\right)^4=4\left(\log \left( x\right)\right)$\dotfill
				
				\part[1]  $\displaystyle \log_2 \left( \frac{1}{2} \right) = -1$ \dotfill
				
				\part[1]  $\displaystyle \log(0)=1$ \dotfill
				
				\part[1] $\displaystyle \log(0.01)=-2$ \dotfill
				
				\part[1] $\displaystyle \log(10)=1$ \dotfill
